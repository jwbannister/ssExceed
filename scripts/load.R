...
# pull IID data
query_teom <- paste0("SELECT i.deployment, t.datetime, t.pm10, ", 
                     ...
                     "JOIN met.met_1hour m ",
                     "ON t.deployment_id=m.deployment_id ",
                     "AND t.datetime=m.datetime ",
                     "WHERE (t.datetime - '1 second'::interval)::date ",
                     "BETWEEN '2016-07-01'::date AND '2017-06-30'::date;")
pm_df <- query_db("saltonsea", query_teom)
pm_df <- pm_df %>% arrange(deployment, datetime) %>% filter(!invalid_pm10) %>%
...
query_aqmis <- paste0("SELECT i.deployment, t.datetime, t.pm10, ", 
                     ...
                     "t.datetime - '1 hour'::interval, t.datetime) ", 
                     "AS invalid_wd ",
                     "FROM aqmis.pm_1hour t JOIN info.deployments i ",
                     "ON t.deployment_id = i.deployment_id ", 
                     "JOIN aqmis.met_1hour m ",
                     "ON t.deployment_id=m.deployment_id ",
                     "AND t.datetime=m.datetime ",
                     "WHERE (t.datetime - '1 second'::interval)::date ",
                     "BETWEEN '2016-07-01'::date AND '2017-06-30'::date;")
aqmis_df <- query_db("saltonsea", query_aqmis)
aqmis_df <- aqmis_df %>% arrange(deployment, datetime) %>% filter(!invalid_pm10) %>%
    select(-invalid_pm10, -invalid_wd, -invalid_ws)
...
df2 <- filter(df1, deployment %in% loc_df$deployment)
